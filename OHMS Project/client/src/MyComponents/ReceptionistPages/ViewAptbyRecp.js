...
export class ViewAptbyRecp extends Component {
    ...
    CreateInvoice(aid) {
        ...
        PatientService.showinvoice(aid).then((res) => {
            this.setState({ iid: res.data[0].iid })
                                ...
                                <tbody>
                                    {this.state.appointments.map((appointment) => (
                                        <tr>
                                            ...
                                            <td>{appointment.atimeslot}</td>
                                            <td>{appointment.adate}</td>

                                            <td>
                                                <button type="button" className="btn btn-success btn-sm"
                                                    onClick={() => this.CreateInvoice(appointment.aid)
                                                    }
                                                >Create</button>
                                            </td>
                                            <td>
                                                <button type="button" className="btn btn-success btn-sm"
                                                    ...
                                                    onClick={() => this.viewInvoice(appointment.aid)}
                                                >View</button>
                                            </td>
                                            <td>
                                                <button type="button" className="btn btn-danger btn-sm" onClick={() => this.deleteAppointment(appointment.aid)}>Cancel</button>
                                            </td>
                                        </tr>
                                    ))}
                                </tbody>
