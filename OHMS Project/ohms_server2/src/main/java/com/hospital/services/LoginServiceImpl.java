...
public class LoginServiceImpl implements LoginService {
	...
	public Login authenticate(String email, String password) {
		...
		System.out.println("in authenticate method "+user+" opass " +password);
		String hashedPassword = user.getPassword();
		System.out.println("in authenticate method hashed pass "+hashedPassword);
		//boolean check = BCrypt.checkpw(password, hashedPassword);
		boolean check = (password.equals(hashedPassword))?true:false;
	
		System.out.println("in authenticate method check "+check);
		if(check)
			return user;
		return null;
	}
